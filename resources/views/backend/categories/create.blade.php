...
<div class="panel panel-default">
							<form class="no-margin" action="{{URL::route('categories.create')}}" method="post" id="formValidate1" data-validate="parsley" novalidate>
									{{csrf_field()}}
								...
								<div class="row">
										<div class="col-md-6">
											<div class="form-group {{ $errors->has('name') ? ' has-error' : '' }}">
												...
												<input type="text" placeholder="Tên danh mục" name="name" id="name" value="{{old('name')}}" class="form-control input-sm" required="required">
										...
										<div class="col-md-6">
											<div class="form-group">
                                              ...
                                              <select class="form-control" id="parent_id" name="parent_id">
                                                ...
                                                <?php $helper::parentAdd($categoryAll) ?>
                                              </select>
                                            </div>
										</div>
									</div>
										...
										<div class="col-md-6 {{ $errors->has('category_type') ? 'has-error' : '' }}">
											<div class="form-group">
                                              ...
                                              @foreach(Config::get('constant.category_type') as $k=>$v)
                                               <option value="{{$k}}" {{(old('category_type')==$k)?'selected':''}}>{{$v}}</option>
                                               @endforeach
                                              </select>
